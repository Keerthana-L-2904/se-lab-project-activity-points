...
const [attendanceUploaded, setAttendanceUploaded] = useState({});
  ...
  {activity.status === "Completed" && activity.outside_inside === "Inside" ? (
    attendanceUploaded[activity.actID] ? (
      <>
        <span style={{ color: 'green', fontWeight: 'bold', marginRight: '6px' }}>Uploaded</span>
        <button style={{ padding: "1px", borderRadius: "2px", cursor: "pointer" }} onClick={() => handleAttendance(activity.actID, true)}>
          Update Attendance
        </button>
      </>
    ) : (
      <button style={{ padding: "1px", borderRadius: "2px", cursor: "pointer" }} onClick={() => handleAttendance(activity.actID, false)}>
        Upload Attendance
      </button>
    )
  ) : (
